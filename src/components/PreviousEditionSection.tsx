...
const PreviousEditionSection = () => {
  ...
  return <section id="previous" ref={ref} className={`min-h-screen flex items-center justify-center px-4 py-20 bg-gradient-to-b from-background to-muted/30 relative overflow-hidden transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-20 right-1/4 w-72 h-72 bg-secondary/10 rounded-full blur-3xl animate-float" style={{
        animationDelay: '2s'
      }}></div>
      ...
      <div className="max-w-6xl w-full relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-16">
          {stats.map((stat, index) => <div key={index} className="bg-card border-4 border-primary rounded-3xl p-10 shadow-lg hover:scale-105 transition-transform animate-fade-in" style={{
        ...
        <div className="bg-card border-4 border-secondary rounded-3xl p-10 shadow-lg mb-12">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {judges.map((judge, index) => <div key={index} className="bg-card rounded-2xl p-8 flex flex-col items-center justify-center hover:scale-105 transition-transform shadow-lg border-2 border-border">
                <img src={judge.logo} alt={judge.company} className="h-20 object-contain mb-4" />
